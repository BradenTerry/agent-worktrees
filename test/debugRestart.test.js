...
const { DebugSessionTracker, runWorktreeTarget } = require(
...
/** Let the queued macrotasks (the task's end event) run. */
...
test("a restart relaunches the same configuration, rebuilding first", async () => {
  ...
  const started = await runWorktreeTarget(dir, "wt", "Run API", false, tracker);
...
test("a restart whose relaunch is refused drops the row", async () => {
  ...
  const dir = makeWorktree();
  const tracker = new DebugSessionTracker();
  await runWorktreeTarget(dir, "wt", "Run API", false, tracker);
