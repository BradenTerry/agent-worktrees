...
- **Know who you're talking to** — the agent whose terminal is currently open is
  highlighted in the panel, its worktree card gets a blue outline, and the
  Agents bar is marked when the card is collapsed, so switching between several
  worktrees never leaves you typing to the wrong agent.
